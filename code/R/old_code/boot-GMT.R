...
samples_vax_child <- analysisData$netMFI_data$long_data %>%
        filter(status=="Vaccinee",age<18,day==0) %>%
        distinct(sample,status) %>%
        mutate(`Age Group`="Children")

samples_vax_adult <- analysisData$netMFI_data$long_data %>%
        filter(status=="Vaccinee",age>=18,day==0) %>%
        distinct(sample,status) %>%
        mutate(`Age Group`="Adults")

samples_case_child <- analysisData$netMFI_data$long_data %>%
        filter(status=="Case",age<18,day==2) %>%
        distinct(sample,status) %>%
        mutate(`Age Group`="Children")

samples_case_adult <- analysisData$netMFI_data$long_data %>%
        filter(status=="Case",age>=18,day==2) %>%
        distinct(sample,status) %>%
        mutate(`Age Group`="Adults")


baseline_sero_data <- analysisData$netMFI_data$long_data %>% 
