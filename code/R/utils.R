...
fit_spec_byvax <- function(data,
        ...
        #get data ready
        spec_data <- data %>% filter(truth==0) %>%
                        mutate(vax=ifelse(status=="Vaccinee",1,0))
        ...
        #run stan model
        logitfit<- stan_glmer(seropos_model ~ (1|id)+vax,
                              data = spec_data,
                              family = binomial(link = "logit"),
        )
        spread <- logitfit %>%
...
sim_truth <- function(n_sim, n_survey=1000,coverage, incidence){
        ...
        for(i in 1:n_sim){
                ...
                out_list[[paste0("sim",i)]] <-true_data
        }
...
make_seropos <- function(data, model_name, 
                         sens_df,spec_df,
                         vax_df,vax_day){
        
        TP_df <- filter(data,R==1) %>%
                group_by(sero_id)%>%
                mutate(pos_ID=sample(unique(sens_df$pos_ID),1)) %>%
                left_join(sens_df, by =c("days_ago", "pos_ID")) %>%
                mutate(seropos=rbinom(1,1,ind_sens)) %>%
        ...
        TN_df_novax <- filter(data,R==0,V==0) %>%
                group_by(sero_id)%>%
                mutate(neg_ID=sample(unique(spec_df$neg_ID),1)) %>%
                left_join(spec_df,by ="neg_ID") %>%
                mutate(seropos=1-rbinom(1,1,ind_spec))%>%
        ...
        colnames(sero_data) <- c("sero_id",model_name)
        
        #output final
        left_join(data,sero_data,by = "sero_id") %>%
                mutate(campaign_day=vax_day)
