...
extractData<- function(file,token=NULL){
  if(!is.null(token)){
    #read in .CSV from dropbox
    ...
    #Local
    trial <- read.csv(file,
                      col.names=paste("column", 1:17, sep="_"),
                      colClasses  = "character"
    )
  }
  ...
  dataTypes_index <- which(trial$column_1=="DataType:")
...
getWideData <- function(){
  ...
  final <- bind_rows(weill_data2,vax_adult_demo2,vax_R1_inventory) %>%
...
getLongLuminexRaw <- function(reload=TRUE){
  ...
  if(reload){
    ...
    file_list <- list.files(paste0(set_paths()$data,"/raw_plate_data/summaryFI"))
    ...
    for (i in 1:nrow(file_list)){
      ...
      run_list[[file_list[i]]] <- extractData(paste0(set_paths()$data,"/raw_plate_data/summaryFI/",file_list[i]))
    }
    
    #get data frames ready
    netmfi_df <-  run_list[[1]]$`Net MFI` %>%
      mutate(path=paste0(set_paths()$data,"/raw_plate_data/summaryFI/",file_list[1]))
    ...
    for (i in 2:length(run_list)){
      netmfi_df <- bind_rows(netmfi_df,
                             run_list[[i]]$`Net MFI`%>%
                               mutate(path=paste0(set_paths()$data,"/raw_plate_data/summaryFI/",file_list[i])))
    ...
    layout_metadata <- read_excel(paste0(set_paths()$data,"/layout_matching.xlsx"),
                                  sheet=1,
                                  # col_names=FALSE,
                                  col_types="text") %>%
      select(-date)
