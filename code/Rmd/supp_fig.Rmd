...
final_wide %>%
        ...
        mutate(pretty_cohort=str_replace(cohort,"BGD","Bangladeshi"))%>%
        mutate(pretty_cohort=str_replace(pretty_cohort,"HTI","Haitian"))%>%
        mutate(pretty_cohort=paste0(pretty_cohort,"s")) %>%
                ggplot(aes(x=day,y=id))+
                geom_point()+
                geom_line()+
                facet_grid(pretty_cohort~.,space = "free",scales = "free")+
                scale_x_continuous("Days post first dose vaccination", breaks = c(14,45,
                                                                                  90,180,360))+
                geom_vline(xintercept = 14,col="red",lty=2)+
                theme_cowplot()+
...
final_wide %>%
        filter(status=="Case") %>%
        group_by(id) %>%
        mutate(order=max(day)) %>%
                ggplot(aes(x=day,y=reorder(id,-order)))+
                geom_point()+
                geom_line()+
                scale_x_continuous("Days post symptom onset", breaks = c(90,180,360,720,1080))+
                theme_cowplot()+
                theme(axis.text.y = element_blank())+
                ylab("Participant ID")
...
Baseline antibody measurements from cases (2-4 days after infection) and vaccinees (day of first dose) are shown on the y-axis (A). Ratios of the geometric mean RAU (comparing Bangladeshi cases and vaccinees) are shown on the y-axis with 95% bootstrapped confidence intervals indicated by vertical lines (B).
...
bootDF <- read_rds("data/generated_data/analysis_objects/bootDF.rds")
...
#         mutate(`Age Group`=factor(`Age Group`,levels=c("Children","Adults"))) %>%
#         mutate(isotype=factor(isotype,levels=c("IgG","IgA","IgM")))%>%
#         filter(`Age Group`=="Children")%>%
#         ggplot(aes(y=RAU_value,fill=cohort,col=cohort,x=cohort))+
