...
ts3 <- baseline_sero_data %>%
        ...
        filter(!is.na(diff))%>%
        group_by(antigen_pretty,isotype,status) %>%
        summarize(pct=paste0(100*round(mean(10^(diff)>=4),2),"%")) %>% 
        ungroup() %>% 
        left_join(ts3_pv)%>%
        spread(status,pct)%>%
        rename(Antigen=antigen_pretty,
...
for(v in 1:length(formula_list)){
        ...
        ids <- unique(base_data$id)
                ...
                cat("Data: ",q, "Variables: ",names(formula_list)[v],"\n")
                ...
                cutoff_list[[names(formula_list)[v]]][[q]] <- median_cut
...
final_wide %>%
        # filter(day>=7)%>%
        mutate(age_group=ifelse(age<=5,
                                "<=5 yrs",">5 yrs"
                                ))%>%
        group_by(age_group,cohort)%>%
        mutate(n=n(),
               r=cor(`RAU_IgG_LT-B`,
                     RAU_IgG_CtxB,method="pearson")) %>%
        ggplot(aes(x=`RAU_IgG_LT-B`,y=RAU_IgG_CtxB))+
        geom_point()+
        geom_label(aes(label=round(r,2),x=-4,y=-2.5))+
        geom_smooth(method="lm",se=FALSE,col="red")+
        facet_grid(age_group~cohort)
...
#         filter(day>=7)%>%
#         filter(age<=5)%>%
#         lmer(formula=RAU_IgG_CtxB~`RAU_IgG_LT-B`*factor(cohort)+(1|id)
#            ) %>%
#         summary()
# 
# final_wide %>%
