...
plot_grid(
        plot_grid(p1,p2,nrow=1,rel_widths = c(0.8,1.2), labels=c("A","B")),
        plot_grid(p3,p4,nrow = 1,rel_widths = c(0.8,1.2), labels=c("C","D")),
        nrow=2
        )
...
lambda_df  %>%
        ...
        geom_boxplot(aes(y=lambda,x=new_coverage,col=adjustment),fill=NA)+
        # geom_violin(aes(y=value,col=type,alpha=type),position="identity",fill="grey",
        #             trim=FALSE,scale="area")+
        # scale_color_manual(values=c("blue",NA),na.value = NA)+
        # scale_alpha_manual(values=c(0,0.3))+
        # geom_point(aes(y=avg_lambda),col="blue")+
        facet_wrap(fct_rev(campaign_day)~.)+
        cowplot::theme_cowplot()+
        # theme(axis.text.x = element_text(angle=45,hjust = 1),
        #       legend.position = "none"
        #       )+
