...
#data for figure 1
fig1_df <- visualization_set_long %>% 
...
fig1_df %>%
        ...
        mutate(Vaccinee=case_when(
                ...
                cohort== "BGD Vaccine" & age>=10 ~ "Bangladeshi 10+ years",
                TRUE ~ cohort
        )) %>%
...
f2a <- misclassify_df2 %>%
        ...
        ggplot(aes(col=Vaccinee,
                   y=seropos,x=day))+
        geom_point(aes(shape=Vaccinee))+
        geom_line() +
        facet_grid(.~end_window)+
        theme_cowplot()+
        scale_x_continuous("Infection Window (Days)",breaks=seq(0,365,90))+
