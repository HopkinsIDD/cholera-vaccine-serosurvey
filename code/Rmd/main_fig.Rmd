...
p2<- bind_rows(p2_outside_df,p2_vax_df)%>%
        mutate(Model=factor(variables,
                            ...
                            labels=c("Case-only model",
                                     "Mixed-cohort model",
                                     "Mixed-cohort 3-class model"
                                     )
                            )) %>%
...
graph_altlambda_df<- lambda_df  %>%
                                filter(adjustment=="Alternative") 
                        


bind_rows(lambda_df %>% filter(adjustment!="Alternative"),
          graph_altlambda_df %>% mutate(campaign_day=21),
          graph_altlambda_df %>% mutate(campaign_day=120)
          )  %>%
        # mutate(campaign_day=ifelse(adjustment=="Alternative",21,campaign_day))%>%
        # mutate(new_strategy=paste(adjustment,campaign_day)) %>%
        # mutate(campaign_day=glue::glue("{campaign_day} days post campaign start"))%>%
        ...
        mutate(new_adjustment=factor(new_adjustment,
                                     labels=c("No adjustment\nfor vaccination",
                                              "Adjust estimates after\nknowing vaccination status",
                                              "Predict with\nMixed-Cohort Model"
                                              )
                                     ))%>%
        # group_by(new_strategy,coverage)%>%
