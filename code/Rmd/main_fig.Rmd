...
f2a <- misclassify_df %>%
        ...
        geom_ribbon(data=filter(tvfpr_df,population== "All Vaccinees")%>%
                        mutate(end_window=factor(end_window,
                                 levels=c("45","120","200","300"),
                                 labels=c("45-day","120-day","200-day","300-day"))),
                        aes(x=time,y=median,ymin=lb,ymax=ub),col=NA,
                                    alpha=0.2,
                        )+
...
```{r f3-loocv, eval=FALSE,fig.width=5.5,fig.height=9}
...
f3a <- loocv_df %>%
        filter(cohort=="SMIC/PIC") %>%
        ggplot()+ 
        plotROC::geom_roc(aes(m = prediction, d = as.numeric(as.character(inf_200)), col=paste(variables,base_data_name)),labels=FALSE,n.cuts=0)+ 
